...
- **Formalized Solid Outer Boundaries**:
  - Components, interactive cards, panels, and chrome must feature crisp, solid high-contrast outlines to separate them unambiguously from background surfaces (preventing "formless" black-on-ash mud):
    ...
    - Bottom Navbar: Container `border-t-2 border-[#FFFFFF]/30`. Navigation items render clean, un-enclosed standalone icons (never wrap single navigation icons in bulky box enclosures). Active items use their section-specific highlight color for the icon, label, and drop-glow (e.g. Note/Ideas: `#EC4899`, Vault: `#10B981`, Goals: `#A855F7`, Settings: `#6366F1`) with an active indicator dot. Grouped enclosures are reserved strictly for grouped trigger components (e.g., top-left Flow + Moments + Hangouts group).

...
| Workspaces | `/app` (active filter) + share `/workspace/[id]` | **Never** link to `/workspaces` — dead. Switcher lives in chrome/sidebar |
...
`/login`, `/handoff`, `/resume`, `/silent-check`, `/connect/settings`, `/vault/sharing`, `/vault/overview`, `/app/extensions`, `/app/settings`, `/app/landing`, `/app/shared`, `/app/popout`, `/r/[username]`, `/project/[projectId]`, `/workspaces`, `/workspaces/**` as product URLs (redirect only), `/workflows` (redirect to `/flows`), accounts subtree as a product shell.

## Helpers

- `isFlowPath` → `/flows` and `/flow/[id]`
- `isGoalsSurfacePath` → goals/forms/events
- `isWorkspacesPath` / `isWorkspaceSharePath` → `/workspace/[id]` share only
- `sanitizeInAppHref` → map dead paths (`/workspaces`, `/workflows`, …) to live ones before `router.push`
...
4. Notification / AI `actionHref` must go through `sanitizeInAppHref`. Default fallback is `/app`, never `/workspaces`.

...
import { readSurfacePref, writeSurfacePref } from '@/lib/ui/surface-memory';
...
export function ConnectMomentsPanel({ onCreateMoment }: ConnectMomentsPanelProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void readSurfacePref(user.$id, 'connect-moments-tab').then((hit) => {
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    void writeSurfacePref(user.$id, 'connect-moments-tab', { id: tab });
  ...
  return (
    <div className="flex flex-col gap-5 w-full max-w-2xl mx-auto min-w-0 overflow-x-hidden">
      <header className="flex flex-col gap-4 px-1 min-w-0">
        <div className="flex items-center justify-between gap-4 min-w-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="w-10 h-10 shrink-0 rounded-xl bg-[#161412] border border-white/20 flex items-center justify-center hover:border-white/40 hover:bg-white/[0.05] transition-colors cursor-pointer"
        ...
        <nav
          className="p-1.5 rounded-2xl bg-[#000000] border-2 border-white/20 flex gap-1 overflow-x-auto scrollbar-none"
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active
                    ? 'bg-[#161412] text-white border border-white/20 shadow-sm'
      ...
      {loading ? (
        <div className="flex flex-col gap-3 min-w-0">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="h-[120px] rounded-2xl bg-[#000000] border-2 border-white/20 animate-pulse"
            />
          ))}
        </div>
      ) : visibleItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center rounded-2xl bg-[#000000] border-2 border-white/20 px-4 space-y-3">
          <div className="w-12 h-12 rounded-2xl bg-[#161412] border border-white/20 flex items-center justify-center mb-1">
            {meta.icon}
          </div>

...
export function ContextMenuPanel({ onCloseAction, items, title }: { onCloseAction: () => void; items: ContextMenuItem[]; appType?: KylrixApp; title?: string }) {
  ...
  return (
    <div
      ...
      className="w-full h-full min-h-0 bg-[#161412] text-white p-5 flex flex-col gap-4 overflow-y-auto font-satoshi"
    ...
    >
      ...
      {isSubmenu && (
        <button
          ...
          className="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-[#161412] border border-[#1C1A18] text-[#9B9691] hover:text-white transition-all text-left text-xs font-bold uppercase tracking-wider mb-1 cursor-pointer"
        >
          <ChevronLeft size={16} />
          <span>Back</span>
        </button>
      ...
      <div className="flex flex-col gap-2">
        {currentItems.map((item, index) => (
          <button
            ...
            className={`w-full flex items-center justify-between gap-3.5 p-3 rounded-2xl text-sm font-semibold transition-all duration-200 text-left border cursor-pointer ${
...
export function ContextMenu({ onCloseAction, items, appType, title }: ContextMenuProps) {
  ...
  return createPortal(
    <>
      ...
      <div
        data-kylrix-context-menu="true"
        className="fixed bottom-0 left-0 right-0 h-[60vh] max-h-[600px] bg-[#161412] border-t border-white/10 rounded-t-[28px] z-[10100] text-white p-5 flex flex-col gap-4 animate-slide-up overflow-y-auto font-satoshi shadow-[0_-24px_48px_rgba(0,0,0,0.8)] max-w-xl mx-auto"
        onClick={(e) => e.stopPropagation()}
        onContextMenu={(e) => e.preventDefault()}
      >

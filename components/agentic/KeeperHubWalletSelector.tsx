...
export function KeeperHubWalletSelector({
  compact = false,
  onOpenDrawer,
}: {
  compact?: boolean;
  ...
  useEffect(() => {
    ...
    if (stored !== null) setTestnetMode(stored === '1');
  ...
  useEffect(() => {
    const handleAccountChange = (e: CustomEvent) => {
      if (e.detail?.account) setActiveWallet(e.detail.account);
    };
    const handleTestnetChange = (e: CustomEvent) => {
      if (typeof e.detail?.testnetMode === 'boolean') setTestnetMode(e.detail.testnetMode);
    };
    window.addEventListener('kylrix:wallet-account-changed' as any, handleAccountChange);
    window.addEventListener('kylrix:wallet-testnet-toggle' as any, handleTestnetChange);
    return () => {
      window.removeEventListener('kylrix:wallet-account-changed' as any, handleAccountChange);
      window.removeEventListener('kylrix:wallet-testnet-toggle' as any, handleTestnetChange);
    };
  }, []);
...
export function KeeperHubWalletDrawer({ onClose }: { onClose: () => void }) {
  const [testnetMode, setTestnetMode] = useState<boolean>(true);
  const [activeWallet, setActiveWallet] = useState<WalletAccount>(DEMO_TESTNET_WALLETS[0]);
  ...
  return (
    <div
      className="absolute inset-0 bg-black/60 z-50 flex flex-col justify-end transition-opacity duration-300"
      onClick={onClose}
    >
      <div
        className="bg-[#0B0A09] border-t border-white/10 rounded-t-[20px] w-full max-h-[60%] flex flex-col overflow-hidden animate-slide-up"

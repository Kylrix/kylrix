...
const thread_STORAGE_KEY = 'kylrix_thread_notes_v2';
const thread_SECRET_KEY = 'kylrix_thread_secret_v2';

/**
 * Background component that claims thread notes when a user authenticates.
...
export const ThreadNoteClaimer = () => {
    ...
    useEffect(() => {
        ...
        const claimthreadNotes = async () => {
            const historyRaw = localStorage.getItem(thread_STORAGE_KEY);
            ...
            try {
                ...
                console.log(`[threadClaimer] Detected ${history.length} items to claim for user ${user.$id}`);
                ...
                for (const item of history) {
                    try {
                        ...
                        const decryptedTitle = item.decryptionKey 
                            ? await decryptThreadData(item.title, item.decryptionKey)
                            : item.title;
                        const decryptedContent = (item.content && item.decryptionKey)
                            ? await decryptThreadData(item.content, item.decryptionKey)
                            : (item.content || '');
                        ...
                        const isUpdate = item.id && !item.id.startsWith('thread-');
                        ...
                        console.error(`[threadClaimer] Failed to claim item ${item.id}:`, itemErr);
                    }
                }
                ...
                if (remainingHistory.length === 0) {
                    ...
                    localStorage.removeItem(thread_SECRET_KEY);
                    console.log('[threadClaimer] All thread items claimed successfully.');
                } else {
                    localStorage.setItem(thread_STORAGE_KEY, JSON.stringify(remainingHistory));
                }

                // Dispatch event to refresh UI
                window.dispatchEvent(new Event('storage'));
                window.dispatchEvent(new CustomEvent('kylrix:thread-claimed'));
            } catch (e) {
                console.error('[threadClaimer] Error processing thread history:', e);
            } finally {

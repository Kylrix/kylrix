...
interface ZapDrawerProps {
  ...
  targetKind?: 'note' | 'goal' | 'event' | 'form' | 'flow' | 'chat';
...
export function ZapDrawer({
  ...
  const ensureVaultReady = async (): Promise<boolean> => {
    ...
    if (!isUnlocked || confirmEveryTime) {
  ...
  const handleSendZap = async () => {
    ...
    if (!vaultOk) {
      toast.error('Confirmation needed to zap');
      return;
    }

    const ecoId = ecosystemTargetId || targetId;
    ...
    if (!ecoId) {
      toast.error('Missing target to zap');
      return;
    }

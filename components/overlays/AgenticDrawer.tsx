    ...
    <>
      ...
      <div
        className={`fixed flex flex-col overflow-hidden bg-[#161412] border-[#34322F] shadow-2xl font-satoshi transition-all duration-300 ${
          isFullscreen
            ...
            : 'z-[1301] bottom-0 left-0 right-0 max-h-[60dvh] h-[60dvh] border-t rounded-t-[24px] md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-[min(100%,520px)] md:rounded-[28px] md:bottom-4 md:border md:max-h-[min(60dvh,720px)]'
        }`}
        style={{
      ...
      >
        {!isDesktop && !isFullscreen && (
          <div className="flex justify-center py-3 flex-shrink-0 select-none">
            <div className="w-10 h-1 rounded bg-[#3D3A36]" />
          </div>
        )}

        <div className="flex items-center justify-end gap-1.5 px-4 pt-1 flex-shrink-0 md:pt-3 md:px-5">
          {!isDesktop && (
            <button
              type="button"

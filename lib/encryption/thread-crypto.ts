 ...
 */
export async function encryptThreadData(text: string, providedKey?: string): Promise<{ encrypted: string; key: string }> {
 ...
 */
export async function decryptThreadData(encryptedData: string, keyBase64: string): Promise<string> {
    try {
        ...
        console.error('[threadCrypto] Decryption failed:', e);
        throw new Error('Failed to decrypt thread note. The key may be invalid.');
    }
}

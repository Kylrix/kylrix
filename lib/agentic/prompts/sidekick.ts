...
export function buildSidekickSystemInstruction(target: SidekickTarget): string {
  return [
    ...
    'FOLLOW-UP CONVERSATION: On subsequent turns, respond as an intelligent, helpful AI assistant. Always answer the user\'s questions directly and thoroughly in markdown format. DO NOT return JSON when answering user questions or follow-ups unless explicitly requested to update a document schema.',
    ...
    'Standard conversational responses must be clean, readable markdown with bold key terms, clear lists, and actionable insights.',

...
import { getFeatureLabel, getFeatureMinTier, tierMeetsFeature, type FeatureId } from '@/lib/tools/features';
...
export async function assertActorFeatureAccess(
  ...
  const err = new Error(
    `${label} requires ${minTier === 'TEAMS' ? 'Teams' : 'Pro'} or higher. Upgrade to continue.`,
  );
  ...
  (err as Error & { minTier?: string }).minTier = minTier;
  throw err;
}
...
export async function checkActorFeatureAccess(
  userId: string,
  featureId: FeatureId,
): Promise<{ allowed: boolean; minTier: 'PRO' | 'TEAMS' | null; userTier: BillingUiTier }> {
